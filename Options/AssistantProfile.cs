...
/// system-prompt text, and the model name. Built once, at startup, via
/// <see cref="Load"/>. There is no embedded fallback prompt — SharpBastion
/// refuses to start with an unconfigured persona rather than silently falling
/// back to baked-in text. Every failure mode here is a hard fail with a message
/// that names the resolved path and states the fix, not just the symptom.
///
...
public class AssistantProfile
{
    ...
    private AssistantProfile(string name, string systemPrompt, string modelName)
    {
        Name = name;
    ...
    public static AssistantProfile Load(AssistantOptions options)
    {
        ...
        return new AssistantProfile(name, systemPrompt, options.ModelName);
    }
}